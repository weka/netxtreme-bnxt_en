...
static int __bnxt_tc_del_flow_tf(struct bnxt *bp, void *flow)
{
	...
	if (flow_node->cparms)
		bnxt_ulp_free_mapper_encap_cparams(flow_node->cparms);
...
#endif
{
	...
	new_node->cparms = flow_info.cparms;
	...
	/* flow_info.cparms will be freed during flow destroy */
