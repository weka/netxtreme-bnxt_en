...
static int bnxt_get_num_tpa_ring_stats(struct bnxt *bp)
{
	if (BNXT_SUPPORTS_TPA(bp)) {
		if (bp->max_tpa_v2) {
			...
			return BNXT_NUM_TPA_RING_STATS_P5_THOR2;
		}
...
skip_tpa_ring_stats:
		...
		bnxt_sw_func_stats[RX_OOM_DISCARDS].counter +=
	...
	if (bp->ktls_info) {
		atomic64_t *ktls_stats = &bp->ktls_info->ktls_tx_add_count;

		for (i = 0; i < BNXT_NUM_KTLS_STATS; i++, j++, ktls_stats++)
			buf[j] = atomic64_read(ktls_stats);
	}
skip_ring_stats:
	if (bp->flags & BNXT_FLAG_PORT_STATS) {
				    ...
				    struct ethtool_rx_flow_spec *fs)
{
	...
	if ((flow_type & (FLOW_MAC_EXT | FLOW_EXT)) || (vf && cookie > 0))
		return -EOPNOTSUPP;
...
bnxt_link_modes[__BNXT_LINK_SPEED_END][2][__BNXT_MEDIA_END] = {
	[BNXT_LINK_SPEED_100MB_IDX] = {
		{
...
bnxt_get_link_mode(struct bnxt_link_info *link_info)
{
	...
	if (sig_mode > BNXT_SIG_MODE_PAM4)
		return BNXT_LINK_MODE_UNKNOWN;
...
static const u16 bnxt_nrz_speed_masks[] = {
	...
	[__BNXT_LINK_SPEED_END] = 0 /* make any legal speed a valid index */
};
...
bnxt_force_link_speed(struct net_device *dev, u32 ethtool_speed, u32 lanes)
{
	...
	case SPEED_50000:
		...
		break;
	case SPEED_100000:
		if ((support_spds & BNXT_LINK_SPEED_MSK_100GB) && lanes != 2) {
