...
static int bnxt_sriov_enable(struct bnxt *bp, int *num_vfs)
{
	...
	if (BNXT_CHIP_THOR(bp) && bp->chip_rev == 1 &&
		...
		max_vf_msix = hw_resc->max_nqs - bnxt_nq_rings_in_use(bp);
...
static int bnxt_hwrm_oem_cmd(struct bnxt *bp, struct bnxt_vf_info *vf)
{
	...
	resp.error_code = 0;
	resp.oem_id = PCI_VENDOR_ID_BROADCOM;
...
void bnxt_reset_vf_stats(struct bnxt *bp)
{
	...
	for (vf_idx = 0; vf_idx < num_vfs; vf_idx++) {
		...
		if (bnxt_tc_is_switchdev_mode(bp)) {
			...
			/* Check if the lock is already held. Otherwise
			 * it could result in a deadlock between this thread
			 * (bnxt_sp_task) and bnxt_vf_reps_destroy()-->
			 * __bnxt_close_nic()-->msleep().
			 */
			if (mutex_trylock(&bp->vf_rep_lock)) {
				vf_rep = bp->vf_reps[vf_idx];

				vf_rep->rx_stats.packets += bnxt_add_ring_rx_pkts(sw);
				vf_rep->tx_stats.packets += bnxt_add_ring_tx_pkts(sw);
				vf_rep->rx_stats.bytes += bnxt_add_ring_rx_bytes(sw);
				vf_rep->tx_stats.bytes += bnxt_add_ring_tx_bytes(sw);
