 ...
 *
 * @count:		Number of module subtypes
 * @cfg:		Pointer to rm element config
 * @reservations:	Pointer to resource reservation array
 *
 * Returns
 *   - (n) number of tables in module that have non-zero reservation count.
 */
static int tf_dev_reservation_check(u16 count, struct tf_rm_element_cfg *cfg,
				    u16 *reservations)
{
	u16 *rm_num;
	u16 cnt = 0;
	...
	for (i = 0; i < TF_DIR_MAX; i++) {
		rm_num = (u16 *)reservations + i * count;
		for (j = 0; j < count; j++) {
 ...
 */
static int tf_dev_bind_p4(struct tf *tfp,
			  ...
			  enum tf_wc_num_slice wc_num_slices)
{
	...
	if (no_rsv_flag) {
		netdev_dbg(tfp->bp->dev, "No rm reserved for any tables\n");
		return -ENOMEM;
	}
	...
	if (rc) {
		netdev_dbg(tfp->bp->dev, "IF Table initialization failure\n");
		goto fail;
	}
...
static int tf_dev_unbind_p4(struct tf *tfp)
{
	...
	if (rc) {
		netdev_dbg(tfp->bp->dev, "Device unbind failed, TCAM\n");
		fail = true;
	}

	rc = tf_ident_unbind(tfp);
	if (rc) {
		netdev_dbg(tfp->bp->dev, "Device unbind failed, Identifier\n");
		fail = true;
	}

	rc = tf_tbl_unbind(tfp);
	if (rc) {
		netdev_dbg(tfp->bp->dev, "Device unbind failed, Table Type\n");
		fail = true;
	}

	rc = tf_em_int_unbind(tfp);
	if (rc) {
		netdev_dbg(tfp->bp->dev, "Device unbind failed, EM\n");
		fail = true;
	}
 ...
 *   - (-EINVAL) on parameter or internal failure.
 */
static int tf_dev_bind_p58(struct tf *tfp,
			   struct tf_session_resources *resources,
			   struct tf_dev_info *dev_handle,
			   enum tf_wc_num_slice wc_num_slices)
{
...
static int tf_dev_unbind_p58(struct tf *tfp)
{
	...
	if (rc) {
		netdev_dbg(tfp->bp->dev, "Device unbind failed, SRAM table\n");
		fail = true;
	}
...
int tf_dev_bind(struct tf *tfp, enum tf_device_type type,
		struct tf_session_resources *resources,
		u16 wc_num_slices, struct tf_dev_info *dev_handle)
{
	switch (type) {
	case TF_DEVICE_TYPE_WH:
		dev_handle->type = type;
		return tf_dev_bind_p4(tfp, resources,
				      dev_handle, wc_num_slices);
	case TF_DEVICE_TYPE_THOR:
		dev_handle->type = type;
		return tf_dev_bind_p58(tfp, resources,
				       dev_handle, wc_num_slices);
	default:
		netdev_dbg(tfp->bp->dev, "No such device\n");
		return -ENODEV;
	}
}
...
int tf_dev_bind_ops(enum tf_device_type type, struct tf_dev_info *dev_handle)
{
	...
	case TF_DEVICE_TYPE_THOR:
		...
		break;
	default:
		netdev_dbg(NULL, "%s: No such device\n", __func__);
		return -ENODEV;
	}
