				...
				struct tf_insert_em_entry_parms *parms)
{
	...
	if (!dev->ops->tf_dev_cfa_key_hash)
		return -EINVAL;

	big_hash = dev->ops->tf_dev_cfa_key_hash((u8 *)parms->key,
					TF_P58_HW_EM_KEY_MAX_SIZE * 8);
	key0_hash = (u32)(big_hash >> 32);
	key1_hash = (u32)(big_hash & 0xFFFFFFFF);

	netdev_dbg(tfp->bp->dev, "Key0 hash:0x%04x\n", key0_hash);
	netdev_dbg(tfp->bp->dev, "Key1 hash:0x%04x\n", key1_hash);

	rptr_index = index;
	rc = tf_msg_hash_insert_em_internal_entry(tfp, parms, key0_hash,
						  key1_hash, fw_session_id,
	...
	TF_SET_FIELDS_IN_FLOW_HANDLE(parms->flow_handle, (u32)num_of_entries,
				     0, TF_FLAGS_FLOW_HANDLE_INTERNAL,
				     rptr_index, rptr_entry, 0);
			 ...
			 struct tf_move_em_entry_parms *parms)
{
	...
	rc = tf_msg_move_em_entry(tfp, parms, fw_session_id);
