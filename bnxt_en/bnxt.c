...
#ifdef BNXT_FPGA
	[BCM57608] = { "Broadcom BCM57608 NetXtreme-E 10Gb/25Gb/50Gb/100Gb/200Gb Ethernet" },
	[BCM57604] = { "Broadcom BCM57604 NetXtreme-E 10Gb/25Gb/50Gb/100Gb Ethernet" },
...
#define BNXT_DB_NQ_P5_THOR2(db, idx)					\
			  ...
			  u32 vlan_tag_flags, u32 cfa_action)
{
	...
	*end = 0;
	...
	prod = NEXT_TX(prod);
	txbd->tx_bd_opaque = SET_TX_OPAQUE(txr, RING_TX(NEXT_TX(prod)));
			     ...
			     u32 vlan_tag_flags, u32 cfa_action)
{
	...
	txbd->tx_bd_opaque = SET_TX_OPAQUE(txr, RING_TX(prod + bds));
...
static netdev_tx_t bnxt_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	skb = bnxt_ktls_xmit(bp, skb, &lflags, &kid);
	if (unlikely(!skb))
		return NETDEV_TX_OK;
	...
	prod = NEXT_TX(prod);
	txbd0->tx_bd_opaque = SET_TX_OPAQUE(txr, RING_TX(prod));
			  ...
			  struct bnxt_tpa_info *tpa_info)
{
	...
	if (bp->flags & BNXT_FLAG_RX_CMPL_V2) {
		u32 csum_count = TPA_START_V2_L4_CS_OK_CNT(tpa_info->flags2);

		if (csum_count) {
			skb->ip_summed = CHECKSUM_UNNECESSARY;
#ifdef HAVE_CSUM_LEVEL
			skb->csum_level = csum_count - 1;
#elif defined(HAVE_INNER_NETWORK_OFFSET)
			skb->encapsulation = csum_count > 1;
#endif
		}
		return;
	}
			...
			struct rx_cmp *rxcmp, struct rx_cmp_ext *rxcmp1)
{
	...
	} else if (cmp_type == CMP_TYPE_RX_L2_V2_CMP ||
			     ...
			     struct rx_cmp *rxcmp, struct rx_cmp_ext *rxcmp1)
{
	if (cmp_type == CMP_TYPE_RX_L2_CMP ||
		...
		u32 csum_count = RX_CMP_V2_L4_CS_OK_CNT(rxcmp1);
		...
		if (RX_CMP_V2_L4_CS_ERRS(rxcmp1))
			return true;
	}
	return false;
}
		       ...
		       u32 *raw_cons, u8 *event)
{
	...
	if (cmp_type == CMP_TYPE_RX_L2_TPA_START_CMP ||
	    cmp_type == CMP_TYPE_RX_L2_TPA_START_V2_CMP ||
				 ...
				 u32 *raw_cons, u8 *event)
{
	...
	cmp_type = RX_CMP_TYPE(rxcmp);
	if (cmp_type == CMP_TYPE_RX_L2_CMP ||
	    cmp_type == CMP_TYPE_RX_L2_V2_CMP ||
	    cmp_type == CMP_TYPE_RX_L2_V3_CMP) {
...
static int bnxt_hwrm_dbr_recovery_completed(struct bnxt *bp, u32 epoch)
{
	...
	if (!(bp->fw_cap & BNXT_FW_CAP_DBR_SUPPORTED) || !(BNXT_PF(bp)))
		return -EOPNOTSUPP;
			    ...
			    int budget)
{
	...
	while (1) {
...
#ifdef DEV_NETMAP
			if (BNXT_CHIP_P5(bp) && netmap_tx_irq(bp->dev, txr->txq_index) !=
					NM_IRQ_PASS)
				break;
#endif /* DEV_NETMAP */
		...
		} else if (cmp_type >= CMP_TYPE_RX_L2_TPA_START_V2_CMP &&
			   cmp_type <= CMP_TYPE_RX_L2_TPA_START_V3_CMP) {
...
#ifdef CONFIG_PAGE_POOL
static int bnxt_alloc_rx_page_pool(struct bnxt *bp, struct bnxt_rx_ring_info *rxr)
{
...
int bnxt_set_rx_skb_mode(struct bnxt *bp, bool page_mode)
{
	if (page_mode) {
		...
		if (bp->dev->mtu > BNXT_MAX_PAGE_MODE_MTU(bp)) {
			bp->flags |= BNXT_FLAG_JUMBO;
...
static int bnxt_alloc_stats(struct bnxt *bp)
{
	...
	if (!BNXT_ASIC(bp) && !(bp->flags & BNXT_FLAG_CHIP_P5_THOR2))
		return 0;
...
int bnxt_hwrm_vnic_set_tpa(struct bnxt *bp, u16 vnic_id, u32 tpa_flags)
{
	struct bnxt_vnic_info *vnic = &bp->vnic_info[vnic_id];
...
int bnxt_hwrm_vnic_set_rss_p5(struct bnxt *bp, u16 vnic_id, bool set_rss)
{
	struct bnxt_vnic_info *vnic = &bp->vnic_info[vnic_id];
	struct hwrm_vnic_rss_cfg_input *req;
...
int bnxt_hwrm_vnic_set_hds(struct bnxt *bp, u16 vnic_id)
{
	struct bnxt_vnic_info *vnic = &bp->vnic_info[vnic_id];
...
void bnxt_hwrm_vnic_ctx_free_one(struct bnxt *bp, u16 vnic_id, u16 ctx_idx)
{
...
int bnxt_hwrm_vnic_ctx_alloc(struct bnxt *bp, u16 vnic_id, u16 ctx_idx)
{
...
int bnxt_hwrm_vnic_cfg(struct bnxt *bp, u16 vnic_id, u16 q_index)
{
	struct bnxt_vnic_info *vnic = &bp->vnic_info[vnic_id];
...
void bnxt_hwrm_vnic_free_one(struct bnxt *bp, u16 vnic_id)
{
...
#ifdef CONFIG_BNXT_CUSTOM_FLOWER_OFFLOAD
		if (bp->vnic_info[vnic_id].vnic_meta) {
			bp->vnic_info[vnic_id].vnic_meta->fw_vnic_id = INVALID_HW_RING_ID;
			bp->vnic_info[vnic_id].vnic_meta->meta_valid = false;
			bp->vnic_info[vnic_id].q_index = INVALID_HW_RING_ID;
		}
#endif
	}
}
...
int bnxt_hwrm_vnic_alloc(struct bnxt *bp, u16 vnic_id,
...
static int bnxt_hwrm_vnic_qcaps(struct bnxt *bp)
{
	...
	if (!rc) {
		...
		if (flags &
			...
			bp->flags |= BNXT_FLAG_ROCE_MIRROR_CAP;
		if (flags & VNIC_QCAPS_RESP_FLAGS_RX_CMPL_V2_CAP)
			bp->flags |= BNXT_FLAG_RX_CMPL_V2;
		...
		if (bp->max_tpa_v2) {
			if (BNXT_CHIP_P5_THOR(bp))
				...
				bp->hw_ring_stats_size = BNXT_RING_STATS_SIZE_P5_THOR2;
		}
...
{
	if (bp->flags & BNXT_FLAG_CHIP_P5) {
		int legacy_db_size = DB_PF_OFFSET_P5;
		...
		if (bp->flags & BNXT_FLAG_CHIP_P5_THOR2) {
			...
			legacy_db_size = bp->legacy_db_size;
		} else if (BNXT_VF(bp)) {
			legacy_db_size = DB_VF_OFFSET_P5;
		}
		db->doorbell = bp->bar1 + legacy_db_size;
	} else {
			     ...
			     u32 map_idx, struct bnxt_ring_struct *ring)
{
	...
	db->doorbell = NULL;
	db->db_key64 = 0;
	if (!(bp->flags & BNXT_FLAG_CHIP_P5) || !bp->db_base_wc)
		return;
...
static int bnxt_hwrm_get_rings(struct bnxt *bp)
{
	...
	if (BNXT_NEW_RM(bp)) {
		...
		hw_resc->resv_tx_key_ctxs = le32_to_cpu(resp->alloc_tx_key_ctxs);
		hw_resc->resv_rx_key_ctxs = le32_to_cpu(resp->alloc_rx_key_ctxs);
	}
	hwrm_req_drop(bp, req);
	return 0;
}
...
#else
			req->num_rsscos_ctxs =
				cpu_to_le16(DIV_ROUND_UP(hwr->grp, 64));
#endif
		} else {
...
__bnxt_hwrm_reserve_vf_rings(struct bnxt *bp, struct bnxt_hw_rings *hwr)
{
	...
	if (bp->flags & BNXT_FLAG_CHIP_P5) {
		req->num_cmpl_rings = cpu_to_le16(hwr->cp_p5);
		req->num_rsscos_ctxs = cpu_to_le16(DIV_ROUND_UP(hwr->grp, 64));
	} else {
		req->num_cmpl_rings = cpu_to_le16(hwr->cp);
		req->num_hw_ring_grps = cpu_to_le16(hwr->grp);
...
static bool bnxt_rings_ok(struct bnxt *bp, struct bnxt_hw_rings *hwr)
{
	return hwr->tx && hwr->rx && hwr->cp && hwr->grp && hwr->vnic &&
	       hwr->stat && (hwr->cp_p5 || !(bp->flags & BNXT_FLAG_CHIP_P5));
}
...
static int bnxt_hwrm_func_qcfg(struct bnxt *bp)
{
	...
	bp->legacy_db_size = le16_to_cpu(resp->legacy_l2_db_size_kb) * 1024;

	if (bp->flags & BNXT_FLAG_CHIP_P5) {
		if (bp->flags & BNXT_FLAG_CHIP_P5_THOR2)
			min_db_offset = bp->legacy_db_size;
		else if (BNXT_PF(bp))
...
#ifdef BNXT_FPGA
	else if (BNXT_CHIP_P5_THOR2(bp) && BNXT_PF(bp))
...
void bnxt_free_ctx_pg_tbls(struct bnxt *bp,
			   struct bnxt_ctx_pg_info *ctx_pg)
{
	struct bnxt_ring_mem_info *rmem = &ctx_pg->ring_mem;
...
static int bnxt_backing_store_cfg_v2(struct bnxt *bp)
{
	...
	for (type = 0 ; type < BNXT_CTX_MAX; type++) {
		...
		rc = bnxt_hwrm_func_backing_store_cfg_v2(bp, ctxm, false);
		if (rc)
			return rc;
	}
	return 0;
...
#ifdef BNXT_FPGA
	if ((bp->flags & BNXT_FLAG_CHIP_P5_THOR2) && BNXT_PF(bp)) {
		rc = bnxt_backing_store_cfg_v2(bp);
		if (rc)
			return rc;
	}
...
int bnxt_hwrm_func_resc_qcaps(struct bnxt *bp, bool all)
{
	...
	hw_resc->min_tx_key_ctxs = le32_to_cpu(resp->min_tx_key_ctxs);
	hw_resc->max_tx_key_ctxs = le32_to_cpu(resp->max_tx_key_ctxs);
	hw_resc->min_rx_key_ctxs = le32_to_cpu(resp->min_rx_key_ctxs);
	hw_resc->max_rx_key_ctxs = le32_to_cpu(resp->max_rx_key_ctxs);
...
static int __bnxt_hwrm_func_qcaps(struct bnxt *bp)
{
	...
	if (flags_ext & FUNC_QCAPS_RESP_FLAGS_EXT_KTLS_SUPPORTED)
		bnxt_alloc_ktls_info(bp, le16_to_cpu(resp->max_key_ctxs_alloc));
	else
...
static int bnxt_hwrm_ver_get(struct bnxt *bp)
{
	...
	if (!BNXT_ASIC(bp) && !BNXT_CHIP_P5_THOR2(bp) && bp->pdev->devfn > 1) {
	...
	if (dev_caps_cfg & VER_GET_RESP_DEV_CAPS_CFG_CFA_EEM_SUPPORTED)
		bp->fw_cap |= BNXT_FW_CAP_CFA_EEM;
...
static int bnxt_hwrm_port_qstats_ext(struct bnxt *bp, u8 flags)
{
	...
	if (!rc) {
		...
		if (BNXT_FW_MAJ(bp) < 220 && !BNXT_CHIP_P5_THOR2(bp) &&
...
static int bnxt_hwrm_func_qstats(struct bnxt *bp, struct bnxt_stats_mem *stats,
				 u16 fid, u8 flags)
{
...
skip_rss_ctx:
	...
	rc = bnxt_hwrm_vnic_cfg(bp, vnic_id, 0);
	if (rc) {
...
static int bnxt_hwrm_vnic_update(struct bnxt *bp)
{
	struct hwrm_vnic_update_input *req;
	struct hwrm_vnic_qcfg_output *resp;
	struct hwrm_vnic_qcfg_input *get;
	struct bnxt_vnic_info *vnic;
	int rc;

	vnic = &bp->vnic_info[0];
	vnic->metadata_format = 0;
	rc = hwrm_req_init(bp, req, HWRM_VNIC_UPDATE);
	if (rc)
		return rc;

	req->vnic_id = cpu_to_le32(vnic->fw_vnic_id);
	req->enables = cpu_to_le32(VNIC_UPDATE_REQ_ENABLES_METADATA_FORMAT_TYPE_VALID);
	req->metadata_format_type = VNIC_UPDATE_REQ_METADATA_FORMAT_TYPE_4;
	rc = hwrm_req_send(bp, req);
	if (rc)
		return rc;

	rc = hwrm_req_init(bp, get, HWRM_VNIC_QCFG);
	if (rc)
		return rc;

	get->vnic_id = cpu_to_le32(vnic->fw_vnic_id);
	resp = hwrm_req_hold(bp, get);
	rc = hwrm_req_send(bp, get);
	if (!rc)
		vnic->metadata_format = resp->metadata_format_type;
					   ...
					   netdev_features_t features)
{
	...
	if (bp->ktls_info && bp->vnic_info) {
		if (bp->vnic_info[0].metadata_format !=
...
static void bnxt_timer(unsigned long data)
{
	...
	if (BNXT_CHIP_P5_THOR(bp) && (!bp->chip_rev || bp->dbr.enable) &&
...
static int bnxt_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (BNXT_CHIP_P5(bp)) {
		bp->flags |= BNXT_FLAG_CHIP_P5;
		if (BNXT_CHIP_P5_THOR2(bp))
			bp->flags |= BNXT_FLAG_CHIP_P5_THOR2;
	}
