...
static int
ulp_set_vlan_in_act_prop(struct bnxt_ulp_context *ulp_ctx, u16 fid,
			 struct bnxt_ulp_mapper_create_parms *mapper_params)
{
...
static int
ulp_set_mark_in_act_prop(struct bnxt_ulp_context *ulp_ctx, u16 fid,
			 struct bnxt_ulp_mapper_create_parms *mapper_params)
{
	if (ULP_BITMAP_ISSET(mapper_params->act->bits,
			...
			struct bnxt_ulp_mapper_create_parms *mapper_params)
{
	...
	/* Set uplink VNIC */
	rc = ulp_set_vnic_in_comp_fld(ulp_ctx, ifindex, BNXT_ULP_VF_FUNC_VNIC,
				      mapper_params);
	if (rc)
		return rc;

	/* Set VF VNIC */
	rc = ulp_set_vnic_in_comp_fld(ulp_ctx, ifindex, BNXT_ULP_DRV_FUNC_VNIC,
				      mapper_params);
	if (rc)
		return rc;
			...
			u32 *flow_id)
{
	...
	struct ulp_tc_act_bitmap act = { 0 };
