...
static int ulp_post_process_normal_flow(struct ulp_tc_parser_params *params)
{
	...
	ULP_COMP_FLD_IDX_WR(params, BNXT_ULP_CF_IDX_EM_FOR_IPV6,
			    SUPPORT_CFA_EM_FOR_IPV6);
...
int ulp_tc_parser_implicit_act_port_process(struct ulp_tc_parser_params
					     *params)
{
	...
	/* TBD: Do we need to handle implicit port action in kernel mode ? */
	return BNXT_TF_RC_ERROR;
}
				 ...
				 struct ulp_tc_parser_params *params)
{
	...
	ulp_tc_parser_implicit_act_port_process(params);
...
int ulp_tc_udp_handler(struct bnxt *bp, struct ulp_tc_parser_params *params,
		       void *match_arg)
{
	struct ulp_tc_hdr_bitmap *hdr_bitmap = &params->hdr_bitmap;
...
int ulp_tc_tcp_ports_handler(struct bnxt *bp,
			     struct ulp_tc_parser_params *params,
			     void *match_arg)
{
	struct ulp_tc_hdr_bitmap *hdr_bitmap = &params->hdr_bitmap;
...
int ulp_tc_redirect_act_handler(struct bnxt *bp,
				...
				void *action_arg)
{
	struct ulp_tc_act_bitmap *act = &params->act_bitmap;
...
int ulp_tc_tunnel_encap_ipv4(struct bnxt *bp,
			     struct ulp_tc_parser_params *params,
			     struct ip_tunnel_key *tun_key)
{
...
int ulp_tc_tunnel_encap_ipv6(struct bnxt *bp,
			     struct ulp_tc_parser_params *params,
			     struct ip_tunnel_key *tun_key)
{
	struct bnxt_tc_neigh_key neigh_key = { 0 };
			    ...
			    void *action_arg)
{
	...
	/* Check for valid chain id range */
	if (chain_id < 1 || chain_id > 4095) {
		netdev_dbg(bp->dev,
			   "%s: goto chain: %u is out of range: 1-4095\n",
			   __func__, chain_id);
		return BNXT_TF_RC_ERROR;
	}
	...
	chain_id = cpu_to_be16(ULP_THOR_SYM_CHAIN_META_VAL +
			       (0xfff & chain_id));
	memcpy(&act_prop->act_details[BNXT_ULP_ACT_PROP_IDX_GOTO_CHAINID],
	       &chain_id, BNXT_ULP_ACT_PROP_SZ_GOTO_CHAINID);
			       ...
			       void *action)
{
	...
	case FLOW_ACT_MANGLE_HDR_TYPE_IP6:
		rc = bnxt_tc_set_l3_v6_action_params(bp, params, offset, val);
		if (rc)
			return rc;
		break;
...
int ulp_tc_mangle_act_handler(struct bnxt *bp,
			      ...
			      void *act)
{
	bnxt_tc_parse_pedit(bp, params, act);
	return 0;
}
